...
function App() {
    ...
    const handleSubmit = evt => {
        ...
        if(input.toLowerCase() ===hiragana[current].romanji){
            ...
            setStreak(0);
            setError(`Resposta errada! A resposta correta para ${hiragana[current].hiragana} é ${hiragana[current].romanji}`);
    ...
    return (
        <div className="min-h-screen bg-slate-800 text-white text-center">
            <header className='p-6 mb-8'>
                <h1 className='text-2xl font-bold uppercase'>Quiz Hiragana</h1>
                <div>
                    <p> {streak} / {maxStreak}</p>
                </div>
            </header>

            <div className='text-9xl fnt-bold mb-8'>
            ...
            <div className='mb-8'>
                <form onSubmit={handleSubmit}>
                    ...
                    className='block w-24 mx-auto pb-2 bg-transparent border-b-2 border-b-white outline-none text-center text-6xl'></input>
                </form>
            </div>
